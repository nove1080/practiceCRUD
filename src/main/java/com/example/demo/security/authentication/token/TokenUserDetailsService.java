package com.example.demo.security.authentication.token;

import com.example.demo.security.authentication.authority.Roles;
import com.example.demo.security.exception.AccessTokenInvalidException;
import com.example.demo.support.token.TokenResolver;
	...
	private static List<GrantedAuthority> toAuthorities(String roles) {
		...
		for (String token : tokens) {
			if (!Objects.equals(token, "")) {
				...
				try {
					rtn.add(Roles.roleOf(role).getAuthority());
